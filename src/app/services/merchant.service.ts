...
export class MerchantService {
    ...
    addNewMerchant(merchant: Merchant, merchantRule: MerchantRule) {
        let headers = new Headers({ 'Content-Type': 'application/json' });
        // headers.append('Access-Control-Allow-Origin', 'http://localhost:3001');
        // headers.append('Access-Control-Allow-Credentials', 'true');
        let options = new RequestOptions({ headers: headers });
        return this.http.post('http://95.177.208.232:8080/happyme-webservice/merchant/1',
            JSON.stringify({
                name: merchant.name,
                description: merchant.description,
                phone: merchant.phone,
                backgroundColor: merchant.backgroundColor,
                merchantRuleData: merchantRule
            }), options)
            .map((response: Response) => response.json());
    }

    addMerchantLogo(merchantId: string, image: any) {
        ...
        // headers.append('Access-Control-Allow-Credentials', 'true');
        let options = new RequestOptions({ headers: headers });
        return this.http.post('http://95.177.208.232:8080/happyme-webservice/merchant/logo/' + merchantId,
            fd, options);
    }

    getMerchants() {
        return this.http.get('http://95.177.208.232:8080/happyme-webservice/merchant', this.jwt()).map((response: Response) => response.json());
    }
